...
#[function_component(ListCard)]
pub fn list_card(Datas {category,content,id, title,seo_title,tag,created_at,updated_at,description, avatar_url, name }:&Datas) -> Html {
	...
	let updated_at = updated_at.clone().unwrap_or("-".to_string());
