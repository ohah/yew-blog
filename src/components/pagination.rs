...
pub fn pagination(PageProps {page,total_rows, page_rows, total_page }:&PageProps) -> Html {
  ...
  html! {
    <div class="w-full flex justify-center">
      <nav class="isolate inline-flex -space-x-px rounded-md shadow-sm " aria-label="Pagination">
        <Link<RootRoute> to={RootRoute::List {page : prev_page.to_string()}}>
        ...
        </Link<RootRoute>>
        {
          if *total_page as i32 <= 6 {
            (1..=*total_page).collect::<Vec<_>>().into_iter().map(|i| {
              html!{
                <Link<RootRoute> key={i} to={RootRoute::List {page : i.to_string()}}>
        ...
        <Link<RootRoute> to={RootRoute::List {page : next_page.to_string()}}>
